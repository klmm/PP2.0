<?php
    ...
    include $_SERVER['DOCUMENT_ROOT'] . '/jeux/biathlon/admin/calcule_classements.php';
    // ------------ INCLUDES ----------//
    ...
    // ------------ MAJ PRONOS ----------//
    $sql = "SELECT * FROM biathlon_prono WHERE id_calendrier=?";
    $prep = $db->prepare($sql);
    $prep->bindValue(1,$id_cal,PDO::PARAM_INT);
    $prep->execute();
    $prep->setFetchMode(PDO::FETCH_OBJ);
    
    $sql2 = "UPDATE biathlon_prono SET points_prono=?, score_base=?, nb_trouves=? WHERE id_biathlon_prono=?";
    ...
    $sql4 = "SELECT * FROM biathlon_prono WHERE id_calendrier=?";
    $prep4 = $db->prepare($sql4);
    $prep4->bindValue(1,$id_cal,PDO::PARAM_INT);
    $prep4->execute();
    $prep4->setFetchMode(PDO::FETCH_OBJ);
    
    $sql3 = "UPDATE biathlon_prono SET bonus_nombre=?, score_total=? WHERE id_biathlon_prono=?";
    $prep3 = $db->prepare($sql3);
    
    while($enregistrement4 = $prep4->fetch()){
	$id_prono = $enregistrement4->id_biathlon_prono;
    ...
    $sql6 = "SELECT * FROM biathlon_prono WHERE id_calendrier=? ORDER BY score_total DESC";
    $prep6 = $db->prepare($sql6);
    $prep6->bindValue(1,$id_cal,PDO::PARAM_INT);
    $prep6->execute();
    $prep6->setFetchMode(PDO::FETCH_OBJ);
    
    $sql7 = "UPDATE biathlon_prono SET classement=? WHERE id_biathlon_prono=?";
	...
	$id_prono = $enregistrement6->id_biathlon_prono;
    ...
    // ------------ MAJ CALENDRIER ----------//
    $sql5 = "UPDATE biathlon_calendrier SET classement=?, traite=1, disponible=0 WHERE id_biathlon_calendrier=?";
    ...
    // ------------ MAJ CLASSEMENTS ----------//
    calcule_classements($id_jeu,$id_cal);
    // ------------ MAJ CLASSEMENTS ----------//
