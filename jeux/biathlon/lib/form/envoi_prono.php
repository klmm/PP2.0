<?php
    ...
    // ------------ PRONO DEJA FAIT ? ----------//
    $sql = "SELECT * FROM biathlon_prono WHERE id_calendrier=? AND joueur=?";
    ...
    if($enregistrement){
	$idpronofait = $enregistrement->id_biathlon_prono;
    }
    else{
	$idpronofait = 0;
    }
    ...
    $nb_etoiles_prono = $_SESSION['biathlon_notes'][$id_jeu][$id_cal]['etoiles_max'];;
	...
	$etoiles_coureur = $_SESSION['biathlon_notes'][$id_jeu][$id_cal][$id_coureur_prono];
    ...
    // ------------ ENVOI DU PRONO ----------//
    if ($idpronofait != 0){
	$sql = "UPDATE biathlon_prono SET prono=?, bonus_risque=? WHERE id_biathlon_prono=?";
	$prep = $db->prepare($sql);
	...
	$sql = "INSERT INTO biathlon_prono(id_jeu,id_calendrier,joueur,prono,points_prono,score_base,bonus_nombre,bonus_risque,score_total,classement,nb_trouves) VALUES(?,?,?,?,'',0,0,?,0,0,0)";
	$prep = $db->prepare($sql);
