<?php
	...
	function get_athletes_tab_id($chaine_id){
		...
		$sql = "SELECT * FROM biathlon_athlete WHERE id IN (" . $chaine_id . ")";
		$prep = $db->prepare($sql);
	...
	function get_athletes_tous(){
		...
		//On pr�pare la requ�te pour aller chercher les articles
		$sql = "SELECT * FROM biathlon_athlete";
		$prep = $db->prepare($sql);
		$prep->setFetchMode(PDO::FETCH_OBJ);
		$prep->execute();
	...
	function get_athletes_activite_genre($genre,$date){
		...
		//On pr�pare la requ�te pour aller chercher les articles
		$sql = "SELECT * FROM biathlon_athlete WHERE genre=? AND retraite=0 AND (date_blessure IS NULL OR date_blessure<?)";
		$prep = $db->prepare($sql);
		$prep->setFetchMode(PDO::FETCH_OBJ);
