<?php
	...
	$sql = "SELECT * FROM biathlon_prono WHERE id_jeu=? AND joueur=?";
	$prep = $db->prepare($sql);
	...
	while( $enregistrement = $prep->fetch() )
	{
	    ...
	    $arr[$id_cal]['id_biathlon_prono'] = $enregistrement->id_biathlon_prono;
	...
	if($enregistrement)
	{
	    ...
	    $id_cal = $enregistrement->id_calendrier;
	    $arr['id_biathlon_prono'] = $enregistrement->id_biathlon_prono;
	...
	//On fait la requete sur le login
	$sql = "SELECT * FROM biathlon_prono WHERE id_calendrier=? ORDER BY classement ASC, score_total ASC, joueur ASC";
	$prep = $db->prepare($sql);
	$prep->bindValue(1,$id_cal,PDO::PARAM_INT);
	$prep->execute();
	$prep->setFetchMode(PDO::FETCH_OBJ);
	...
	while ($enregistrement = $prep->fetch())
	{
	    ...
	    $arr[$joueur]['id_biathlon_prono'] = $enregistrement->id_biathlon_prono;
